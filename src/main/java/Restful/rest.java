...
//    ********************** delete specific note by rage id and note id **********************
    ...
    //    ********************************** Get request *****************************
    @ResponseBody
    @RequestMapping(value = "/student/{vcode}", method = RequestMethod.GET, produces = MediaType.TEXT_PLAIN_VALUE)
    public String GetUserByVCode(@PathVariable("vcode") String VCode) {
    ...
    //    ***************************** Put request *******************
    @RequestMapping(value = "/ss/{username}", method = RequestMethod.PUT, consumes = MediaType.APPLICATION_XML_VALUE)
    public @ResponseBody
    ResponseEntity<Users> UpdateStudent(@PathVariable("username") String username, @RequestBody Users users) {


//        userService.updateEnabled(users);
        System.out.println("rest Put ..." + username + " - " + users.getEmailaddress());


        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.add("key1", "val1");
        httpHeaders.add("key2", "val2");

        return new ResponseEntity<Users>(users, httpHeaders, HttpStatus.OK);

    }

    //    ********************************* post*****************
    @ResponseBody
    @RequestMapping(value = "/student/aa", method = RequestMethod.POST, consumes = MediaType.APPLICATION_XML_VALUE)
